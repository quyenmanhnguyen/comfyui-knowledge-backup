...
## NSFW detail/anatomy LoRA + full-edit node audit — 2026-07-26

- User manually downloaded and local validation confirmed five Civitai detail/anatomy LoRAs under `C:\AI\ComfyUI\models\loras\civitai_nsfw_detail_20260726`: `Hentai_Pussy_inspection_Anima_V01.safetensors`, `Hentai_Studio_Quality_Anima-step00001300.safetensors`, `nipple_pull_anima_V1.0.safetensors`, `zimage_better_pussy_poses_v1.0.safetensors`, and `female_anatomy_zib-000085-comfy.safetensors`. All are valid safetensors.
- ComfyUI object info confirms the full-edit toolset is available locally: `face_yolov9c.pt`, `hand_yolov9c.pt`, `vagina-v4.2.pt`, `FaceDetailer`, `UltralyticsDetectorProvider`, `ZImageTurboLoraLoader`, `DiTBlockLoraLoader`, and standard `LoraLoader`.
- Evidence/report: `C:\AI\workflows\FULL_EDIT_DETAIL_PACK_20260726\REPORT.html`. Six no-upscale workflow variants were created under `workflows\`; five were locally queued or validated. This pack is research, not an automatic main promotion.
- Strong reject as default: stacking `Hentai_Studio_Quality_Anima` and `nipple_pull_anima` on Anima/aMix even at reduced strengths produced porcelain/chrome/plastic doll skin, large doll eyes, metallic body highlights, and fake signature/logo artifacts. Do not use these LoRAs as generic quality/detail enhancers for the user's bright natural target.
- Strong reject as default: `Hentai_Pussy_inspection_Anima` at 0.16 on the Ri-mix alpha Anima branch pushed the image into liquid-metal/chrome skin and fake logo/text. Treat it as a very narrow fetish/style LoRA only, not a main workflow ingredient.
- New Z-Image research keep: `female_anatomy_zib-000085-comfy.safetensors` through `ZImageTurboLoraLoader` at strength 0.08 produced `FEDIT-06B_Z_FEMALEANATOMY_LOW008_00001_.png`, the best new detail/anatomy direction in this pass: bright clean face, less muddy skin, natural-ish Z texture. Remaining weakness: hand/finger quality still needs prompt/pose control or targeted rescue.
- Z `zimage_better_pussy_poses_v1.0.safetensors` at strength 0.10 produced a good bright face/skin but caused cramped close crop and breast/anatomy distortion. Keep only as low-strength research with medium-shot prompts; do not promote from one attractive face.
- SDXL photoreal Face+Hand rescue graph hit a ComfyUI model-manager `IndexError` when run immediately after Z in the same process. Treat this as a mixed-family process-state issue; retest SDXL detailers in a fresh process before judging the workflow.
- Current full-edit rule: base winner first, then one low-strength LoRA or one targeted detector pass. Do not combine Anima anatomy LoRAs, face/hand/lower-body detailers, and Z LoRAs in one maximal graph unless each component has passed an A/B visual test.

## Proper retest of downloaded Civitai detail LoRAs — 2026-07-26

- User observed the first full-edit report looked dark/black/chrome and suspected file placement. Retest confirmed file placement was correct: all downloaded safetensors were in `models\loras\civitai_nsfw_detail_20260726` and visible to `LoraLoader`, `ZImageTurboLoraLoader`, and `DiTBlockLoraLoader` after a ComfyUI restart.
- `C:\Users\Admin\Downloads\ccmax-20,最新免费教程脚本.txt` is a Claude checkout userscript, not a ComfyUI model/workflow. Do not treat it as an image-generation resource.
- Metadata explained the failure: `Hentai_Pussy_inspection_Anima_V01` has training tags such as `character profile`, `english text`, `radar chart`, `multiple layers`, and close-up anatomy tags; `Hentai_Studio_Quality_Anima` has trigger `hentai_studio_quality` plus shiny/sweat/gloss tags. These LoRAs are highly style-active and can create fake text, poster/layout artifacts, chrome skin, or plastic skin if used at normal strengths or stacked.
- Evidence/report: `C:\AI\workflows\DETAIL_RETEST_PROPER_20260726\REPORT.html`. Six workflows were tested with clean baselines and one LoRA at a time, no upscale.
- `DRETEST-01_ANIMA_BASE_CLEAN`: Ri-mix alpha Anima baseline, 68.2 s. Cleaner than LoRA variants; no chrome/text. Keep as the control for Anima comparisons.
- `DRETEST-02_ANIMA_STUDIO006`: `Hentai_Studio_Quality_Anima` at 0.06, 60.3 s. Much better than the earlier 0.32 stack, but still plastic/glossy with stiff hands. Use only as a style-only anime glossy option, not the natural main.
- `DRETEST-03_ANIMA_INSPECT004`: `Hentai_Pussy_inspection_Anima_V01` at 0.04, 60.3 s. No fake text/chrome this time, but still game/plastic. Use only style-only; never stack as generic detail.
- `DRETEST-04_Z_FEMALE006`: Z + `female_anatomy_zib` at 0.06, 153.2 s. Best new LoRA detail direction: bright, clean, natural-ish Z face/skin. Downside is long runtime and imperfect hand. Keep as Z HQ research, not draft default.
- `DRETEST-05_Z_POSE005`: Z + `zimage_better_pussy_poses` at 0.05, 128.3 s. Similar face/skin but slightly worse body/crop than female anatomy. Secondary only.
- `DRETEST-06_ONEOBS_CLEAN_MAIN`: OneObsession v23 clean control, 46.7 s. Best anime/illustration winner in this retest: most visually impressive, bright, and coherent without the new anatomy LoRAs. Continue treating OneObsession clean as the main high-impact anime workflow.
- Practical rule update: if testing these Civitai anatomy/detail LoRAs again, start at 0.04–0.08, include negatives for `english text`, `character profile`, `radar chart`, `multiple layers`, `chrome skin`, `metallic skin`, and compare against a no-LoRA control. File path is not the problem; strength and trigger grammar are.

## Full adult NSFW anatomy/detail A/B — 2026-07-26

- Evidence/report: `C:\AI\workflows\FULL_NSFW_DETAIL_AB_20260726\REPORT.html`. Seven controlled adult NSFW workflows were tested: Anima control, Anima + Studio, Anima + Nipple, Anima + Lower Inspection, OneObsession control, Z + Female Anatomy, and Z + Better Pose. No upscale was used.
- Main winner: `05_ONEOBS_CONTROL_FULL.json` / output `FNSFW-05_ONEOBS_CONTROL_FULL_00001_.png`. It is the best current anime full-NSFW workflow for the user's "căng/xịn/ấn tượng" target: attractive face, strong light, coherent chest/nipple detail, and high-impact style. It does not show lower-body detail much because the crop is close, so use a wider prompt if lower anatomy is required.
- Secondary usable: `02_ANIMA_STUDIO008.json` / `FNSFW-02_ANIMA_STUDIO008_00001_.png`. `Hentai_Studio_Quality_Anima` at 0.08 improved chest/nipple detail and face over the Anima control. Weakness: still slightly plastic and hands are soft. Keep as secondary glossy Anima NSFW.
- Niche usable: `03_ANIMA_NIPPLE006.json` / `FNSFW-03_ANIMA_NIPPLE006_00001_.png`. `nipple_pull_anima` at 0.06 gives the clearest chest/nipple emphasis but makes the face older/western and hands weaker. Use only for chest-detail-specific anime tests.
- Reject as main: `01_ANIMA_CONTROL_FULL` had obvious breast/anatomy failure; `04_ANIMA_LOWER0035` did not produce better lower-body detail and made the face/tone older/plastic. The lower-inspection LoRA is not a reliable full-body/lower-detail fix in this setup.
- Z result: `06_Z_FEMALEANAT0055` is clean photoreal-soft but took 307.2 s and did not strongly improve lower-body detail. `07_Z_POSE004` was near-duplicate and also very slow. Do not use Z anatomy LoRAs as main full-NSFW detail workflows on this 16 GB machine unless the user explicitly wants slow photoreal-soft research.
- Practical next rule: for full adult NSFW detail, use OneObsession clean as the main anime base; use Anima Studio 0.08 or Nipple 0.06 only as secondary LoRA branches. For lower-body/pussy detail, the currently downloaded lower-inspection LoRA does not beat the main base; try a wider OneObsession prompt or a regional detector/detail pass only after a controlled A/B.

## SDXL/Anima nipple/lower-detail A/B — 2026-07-26

- User asked to skip Z-Image and focus only SDXL/Illustrious/Anima NSFW nipple/lower-detail improvement. Ten no-upscale API workflows were created and tested under `C:\AI\workflows\SDXL_ANIMA_DETAIL_FOCUS_20260726`; report: `REPORT.html`; selected reusable files: `SELECTED`.
- Main close-up winner: `02_ONEOBS_CHEST_SOFA_CLEAN.json` / `XAD-02_ONEOBS_CHEST_SOFA_CLEAN_00001_.png` at 22.1 s. It is the strongest high-impact OneObsession anime NSFW close-up: attractive face/eyes, strong chest/nipple detail, good lighting. Weakness: very glossy/wet skin and close framing.
- Main lower/framing candidate: `08_ONEOBS_SEATED_LOWER_CLEAR.json` / `XAD-08_ONEOBS_SEATED_LOWER_CLEAR_00001_.png` at 22.1 s. Best batch result for lower-body framing while keeping face/body impact. Weakness: still glossy and hands are mostly hidden; do not treat it as hand validation.
- Main softer Anima candidates: `09_ANIMA_STUDIO004_FULLBODY.json` and `10_ANIMA_NIPPLE0025_SOFT.json`. `Hentai_Studio_Quality_Anima` at 0.04 produced the best Anima face/eyes; `nipple_pull_anima` at 0.025 gave the cleanest softer full-body-ish chest emphasis. Both remain anime-glossy but avoid the chrome/black failures seen in earlier high-strength tests.
- Secondary/niche: `03_ANIMA_RIMIX_STUDIO006.json` works without chrome at 0.06 and improves chest/face but is more doll-like. `04_ANIMA_RIMIX_NIPPLE004.json` gives the clearest nipple emphasis at 0.04 but hides hands and makes the face more generic; use only for chest-detail-specific experiments.
- Reject as main: `05_ANIMA_RIMIX_INSPECT002.json` still shifts eyes/style and does not beat OneObsession for lower-body detail, even at 0.02. `07_ONEOBS_FULLBODY_PULLBACK.json` ignored the requested full-body framing and cropped the face. Do not keep pushing these exact directions by prompt alone.
- Operational rule: for this user's current “căng/xịn” adult-anime target, start from OneObsession clean for visual impact. Add Anima Studio only at 0.04–0.06, Nipple only at 0.025–0.04, and avoid stacking with Inspection by default. High-strength or stacked nipple/pussy LoRAs repeatedly cause chrome/plastic skin, generic/red eyes, fake text, or worse anatomy.
